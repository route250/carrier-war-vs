...
class TestAICarrierBotAll(unittest.TestCase):
    ...
    def bot_setup(self, provider: str, model: str) -> LLMBase|None:
        ...
        if not bot._ensure_client_ready():
            self.skipTest(f"Skipping {provider} LLM bot test because API key is not set or client cannot be initialized.")
            return
        bot.stat = AIStat.READY
        if not bot.startup():
            self.skipTest(f"Skipping {provider} LLM bot test because startup() failed.")
            return
    ...
    def is_bot_launches_and_carrier_moves(self,bot: LLMBase) -> bool:
        ...
        self.assertTrue(launched, "LLMボットが発艦（攻撃/索敵行動）しませんでした")
        self.assertTrue(moved, "LLMボットの空母が移動しませんでした")
        return False

    def _run_llm(self, provider, model):
        bot = self.bot_setup(provider, model)
        if bot is None:
            print(f"\nSkipping test for {provider} model {model} due to setup failure.")
            return
        self.is_bot_launches_and_carrier_moves(bot)

    def test_all_llm(self):
        skips = [
         # "openai", "anthropic",
         #"gpt-4.1-mini", "gpt-4.1", "gpt-4o-mini", "gpt-4o", "o4-mini", "o3-mini", "o3", "gpt-5-nano", "gpt-5-mini", "gpt-5", "gpt-3.5-turbo",
         # OpenAI: 有料APIで高価なためスキップ
        ]
        limit:int = 1
        test_case = []
        pp = [ CarrierBotOpenAI.get_model_names(), CarrierBotAnthropic.get_model_names(), CarrierBotGemini.get_model_names() ]
        for p in pp:
            if p.name.lower() in skips:
                continue
            cnt:int = 0
            for m in p.models:
                if m.name in skips:
                    continue
                test_case.append( (p.name.lower(), m.name) )
                cnt += 1
                if cnt >= limit:
                    break
        ...
        # test_case = [ ("gemini", "gemini-2.5-flash-lite") ]
        # test_case = [ ("anthropic", "Claude Haiku 3") ]
        for provider, model in test_case:
            self._run_llm(provider, model)
